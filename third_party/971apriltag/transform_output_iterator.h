...
class TransformOutputIterator {
 ...
 public:
  ...
  __host__ __device__ __forceinline__ reference operator[](IntT num) const {
    return Reference(ptr + num, convert_op);
  }

  // pointer arithmetic
  template <typename IntT>
  __host__ __device__ __forceinline__ self_type operator+(IntT n) const {
    return self_type(ptr + n, convert_op);
  }
